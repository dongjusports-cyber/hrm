@echo off
chcp 65001 >nul
title Nap 360 NV tu HIEN_PHAP/trich_xuat
cd /d "%~dp0.."

echo.
echo === Nap ho so NV tu trich_xuat_140826 ===
echo.
...
set "TRICH=HIEN_PHAP\Thông tin danh sách nhân viên\trich_xuat_140826"
if not exist "%TRICH%\employees.json" (
  echo Khong thay %TRICH% — thu trich_xuat_110826...
  set "TRICH=HIEN_PHAP\Thông tin danh sách nhân viên\trich_xuat_110826"
)

echo Copy du lieu vao container...
docker cp "%TRICH%" djhrm-api:/tmp/trich_xuat
for %%D in ("HIEN_PHAP\Thông tin danh sách nhân viên") do docker cp "%%~fD" djhrm-api:/tmp/empinfo
